...
func StartServer(
	...
	// Concepts resolve from an in-process table by default, which needs no OCL
	// account. Set TERMINOLOGY_BACKEND=ocl to call an OpenConceptLab server.
	var ocl infrastructure.ServiceOCL

	if os.Getenv("TERMINOLOGY_BACKEND") == "ocl" {
		ocl = openconceptlab.NewServiceOCL()
	} else {
		localOCL, err := openconceptlab.NewLocalService()
		if err != nil {
			log.Panicf("failed to load the local terminology: %v", err)
		}
	...
	// The shortener is a SIL-hosted service. Without it links stay long, which the
	// shortener service handles, so only build a client when one is configured.
	var silURLShortnerClient urlshortener.IURLShortenerClient

	if os.Getenv(urlshortener.DomainEnvVarName) != "" {
