...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Questionnaire":
		resolverName, err := entityResolverNameForQuestionnaire(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Questionnaire": %w`, err)
		}
		switch resolverName {

		case "findQuestionnaireByID":
			id0, err := ec.unmarshalNString2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findQuestionnaireByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindQuestionnaireByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Questionnaire": %w`, err)
			}

			return entity, nil
		}
	case "ServiceRequest":
		resolverName, err := entityResolverNameForServiceRequest(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "ServiceRequest": %w`, err)
		}
		switch resolverName {

		case "findServiceRequestByID":
			id0, err := ec.unmarshalNString2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findServiceRequestByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindServiceRequestByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "ServiceRequest": %w`, err)
			}
...
func entityResolverNameForQuestionnaire(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Questionnaire", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Questionnaire", ErrTypeNotFound))
			break
		}
		return "findQuestionnaireByID", nil
	}
	return "", fmt.Errorf("%w for Questionnaire due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForServiceRequest(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for ServiceRequest", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for ServiceRequest", ErrTypeNotFound))
			break
		}
		return "findServiceRequestByID", nil
	}
	return "", fmt.Errorf("%w for ServiceRequest due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
